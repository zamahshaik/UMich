...
plt.figure()
plt.title('Sample binary classification with 2 informative features')
...
plot_class_regions_for_classifier(nbclf, X_train, y_train, X_test, y_test, 'Gaussian NB classifier: Dataset 1')
...
nbclf = GaussianNB().fit(X_train, y_train)
plot_class_regions_for_classifier(nbclf, X_train, y_train, X_test, y_test, 'Gaussian NB Classifier: Dataset 2')
...
for pair, axis in zip(pair_list, subaxes):
    ...
    axis.set_label(feature_names_fruits[pair[0]])
    axis.set_label(feature_names_fruits[pair[1]])
...
# ### Gradient boosted decision tree on a fruit dataset 
...
print('Accuracy for GBDT classifier on training set: {:.2f}'.format(clf.score(X_train, y_train)))
print('Accuracy for GBDT classifier on test set: {:.2f}'.format(clf.score(X_test, y_test)))
...
print('Brest cancer dataset(learning rate = 0.01, max_depth = 2)')
print('Accuracy of GBDT classifier on training set: {:.2f}'.format(clf.score(X_train, y_train)))
print('Accuracy of GBDT classifier on test set: {:.2f}'.format(clf.score(X_test, y_test)))
...
for this_alpha, axis in zip([0.01, 0.1, 1.0, 5.0], subaxes):
    ...
    title = 'Dataet 2: NN classifier, alpha = {:.3f}'.format(this_alpha)
